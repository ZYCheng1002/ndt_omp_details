...
class NormalDistributionsTransform : public pcl::Registration<PointSource, PointTarget> {
 ...
 public:
  ...
  inline void setResolution(float resolution) {
    ...
    if (resolution_ != resolution) {
      ...
      /// 已经存在输入(inputSource),必须进行初始化,值得注意的是inputTarget改变resolution不影响
      if (input_) init();
    }
  }
 ...
 protected:
   ...
   * \param[in] p 当前变换向量, [x, y, z, roll, pitch, yaw]
   * \param[in] compute_hessian 是否计算H阵, 默认true, unnecessary for step calculation.
   */
  ...
  /** \brief 计算残差对变换的雅可比和海森矩阵(带入点,如公式)
   ...
   * estimate, \f$ \alpha_0 \f$ in Moore-Thuente (1994) and the normal of \f$ \delta \vec{p} \f$ in Algorithm 2
   ...
   * Moore-Thuente (1994), \f$ \psi'(\alpha_u) \f$ for Update Algorithm and \f$ \phi'(\alpha_u) \f$ for Modified Update
   * Algorithm \param[in] a_t trial value, \f$ \alpha_t \f$ in Moore-Thuente (1994) \param[in] f_t value at trial value,
   * \f$ f_t \f$ in Moore-Thuente (1994), \f$ \psi(\alpha_t) \f$ for Update Algorithm and \f$ \phi(\alpha_t) \f$ for
   * Modified Update Algorithm \param[in] g_t derivative at trial value, \f$ g_t \f$ in Moore-Thuente (1994), \f$
   * \psi'(\alpha_t) \f$ for Update Algorithm and \f$ \phi'(\alpha_t) \f$ for Modified Update Algorithm \return if
   * interval converges
   */
  ...
  float resolution_;                       // 分辨率, voxel的边长, 默认1.0
  double step_size_;                       // 最大步长, 默认0.05
  double outlier_ratio_;                   // 外点比例, Equation 6.7 [Magnusson 2009]
  double gauss_d1_, gauss_d2_, gauss_d3_;  // 拟合高斯分布的系数, Equation 6.8 [Magnusson 2009]
  double trans_probability_;               // 概率得分, Equation 6.9 and 6.10 [Magnusson 2009]

  /**  the jacobian of a transformation vector, Equation 6.19 [Magnusson 2009] */
  Eigen::Vector3d j_ang_a_, j_ang_b_, j_ang_c_, j_ang_d_, j_ang_e_, j_ang_f_, j_ang_g_, j_ang_h_;  // 雅可比表示
  Eigen::Matrix<float, 8, 4> j_ang;

  /**  the hessian of a transformation vector, Equation 6.19 [Magnusson 2009] */
  ...
  Eigen::Matrix<double, 3, 6> point_gradient_;  // 雅可比矩阵， \f$ J_E \f$
  Eigen::Matrix<double, 18, 6> point_hessian_;  // 海森矩阵， \f$ H_E \f

  int num_threads_;  // 线程数

 public:
  NeighborSearchMethod search_method;  // 搜索方法
