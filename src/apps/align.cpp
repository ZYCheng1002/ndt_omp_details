...
pcl::PointCloud<pcl::PointXYZ>::Ptr align(pcl::Registration<pcl::PointXYZ, pcl::PointXYZ>::Ptr registration,
  ...
  reg_omp::tools::Timer::Evaluate([&]() { registration->align(*aligned); }, "align 1, " + reg_name);
  reg_omp::tools::Timer::Evaluate(
      [&]() {
        for (int i = 0; i < 10; i++) {
...
int main(int argc, char** argv) {
  ...
  *target_cloud = *downsampled;
  ...
  pcl::PointCloud<pcl::PointXYZ>::Ptr aligned = align(gicp, "gicp", target_cloud, source_cloud);
  ...
  aligned = align(gicp_omp, "gicp_omp", target_cloud, source_cloud);
  ...
  aligned = align(ndt, "ndt", target_cloud, source_cloud);
  ...
  for (int n : num_threads) {
    for (const auto& search_method : search_methods) {
      ...
      aligned = align(ndt_omp, "ndt_omp_" + search_method.first, target_cloud, source_cloud);
    }
  }
  reg_omp::tools::Timer::PrintAll();
  // visualization
